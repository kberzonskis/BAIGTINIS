...
export function AdminFoodForm({ api, method, food }) {
    const [title, setTitle] = useState(food?.title ?? '');
    const [url, setUrl] = useState(food?.url_slug ?? '');
    const [description, setDescription] = useState(food?.description ?? '');
    const [status, setStatus] = useState(food?.status_name ?? 'draft');

    const { updateAdminFoods, updatePublicFoods } = useContext(FoodsContext);
